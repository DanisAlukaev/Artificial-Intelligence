...
initialize_open_closed_lists(StartingPosition):-
	...
	retractall(open(_)),
	retractall(closed(_)),
	assert(open([(StartingPosition, 0, StartingPosition)])),
	assert(closed([])).
...
process_cell(EvaluatedCurrentCell, NextCell) :-
	%% Candidate cell IS in the open list.
	open(Open),
	unpack_cells(Open, [], OpenCells),
...
a_star() :-
	...
	write(PathWOImmunity), nl,
	write(MaskPathWOImmunity), nl,
	write(DoctorPathWOImmunity), nl,
	...
	closed(DoctorClosed),
	write(DoctorClosed), nl,
	write(DoctorPathImmunity), nl,
