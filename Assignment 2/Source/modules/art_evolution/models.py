...
def _multiprocessing_crossover(parents, number_mutations):
    ...
    offspring = Individual(offspring_chromosome, number_mutations, parent1['individual'].use_palette)
...
class Individual:
    ...
    def __init__(self, chromosome=None, number_of_mutations=1, use_palette=False):
        """
...
class Population:
    ...
    def __init__(self, population_size, elite_size, mutations_number, use_palette):
        """
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=CORES_NUMBER) as executor:
            results = []
            # create progenitor
            individual = Individual(number_of_mutations=mutations_number, use_palette=use_palette)
        ...
        probability_scale = population[0]['fitness'] + population[self.elite_size - 1]['fitness']

        for i in range(self.population_size):
            # assign probability of being selected based on the fitness value of individual
            probability = (probability_scale - population[i]['fitness']) / total_fitness if i < self.elite_size else 0
    ...
    def _cumulative_distribution_function(self):
        ...
        probabilities = [self.population[i]['probability'] for i in range(self.elite_size)]
        # set boundaries
    ...
    def crossover(self):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=CORES_NUMBER) as executor:
            # process parents
            results = [executor.submit(_multiprocessing_crossover, parents, self.mutations_number) for parents in
                       self.parents]
